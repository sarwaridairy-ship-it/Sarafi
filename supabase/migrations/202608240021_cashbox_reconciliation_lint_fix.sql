-- Correct reconciliation function references and cashbox account construction.
...
begin
  ...
  insert into public.cashbox_closes (organization_id, branch_id, cashbox_id, closed_by, business_date, status, variance_reason)
    values (org_id, branch_id_value, cashbox_id_value, actor_id, business_date_value, 'submitted', nullif(trim(command->>'variance_reason'), '')) returning id into new_close_id;
  for count_item in select * from jsonb_array_elements(lines) loop
    ...
    select coalesce(sum(jl.native_debit - jl.native_credit), 0) into expected_value from public.journal_lines jl join public.ledger_accounts la on la.id = jl.account_id where jl.organization_id = org_id and la.cashbox_id = cashbox_id_value and jl.currency_code = currency_value;
    insert into public.cashbox_close_lines (close_id, organization_id, currency_code, expected_amount, counted_amount) values (new_close_id, org_id, currency_value, expected_value, counted_value);
  end loop;
  if exists (select 1 from public.cashbox_close_lines ccl where ccl.close_id = new_close_id and ccl.variance_amount <> 0) and nullif(trim(command->>'variance_reason'), '') is null then raise exception 'A reason is required for cash variance'; end if;
  update public.cashbox_closes cbc set status = case when exists (select 1 from public.cashbox_close_lines ccl where ccl.close_id = new_close_id and ccl.variance_amount <> 0) then 'submitted' else 'approved' end where cbc.id = new_close_id returning cbc.* into result;
  return result;
end;
$$;
...
begin
  ...
  for variance_row in select * from public.cashbox_close_lines where close_id = target_id and variance_amount <> 0 loop
    ...
    insert into public.ledger_accounts (organization_id, code, name, category, currency_code, cashbox_id)
      values (close_row.organization_id, 'cashbox:' || close_row.cashbox_id || ':' || variance_row.currency_code, 'Cashbox ' || variance_row.currency_code, 'asset', variance_row.currency_code, close_row.cashbox_id)
