-- Include realized FX losses in owner reporting instead of silently omitting them.

create or replace view public.organization_realized_profit
with (security_invoker = true)
as
select
  je.organization_id,
  date_trunc('day', je.occurred_at)::date as business_date,
  je.branch_id,
  coalesce(sum(
    case
      when la.code = 'income:realized-fx-gain' then jl.base_credit - jl.base_debit
      when la.code = 'expense:realized-fx-loss' then jl.base_credit - jl.base_debit
      else 0
    end
...
where je.status = 'posted'
  and la.code in ('income:realized-fx-gain', 'expense:realized-fx-loss')
group by je.organization_id, date_trunc('day', je.occurred_at)::date, je.branch_id;
...
begin
  ...
  select jsonb_build_object(
    ...
    'realized_profit', coalesce((select sum(case when la.code = 'income:realized-fx-gain' then jl.base_credit - jl.base_debit when la.code = 'expense:realized-fx-loss' then jl.base_credit - jl.base_debit else 0 end) from public.journal_entries je join public.journal_lines jl on jl.journal_entry_id = je.id join public.ledger_accounts la on la.id = jl.account_id where je.organization_id = target_org and je.occurred_at::date = target_day and je.status = 'posted' and la.code in ('income:realized-fx-gain', 'expense:realized-fx-loss')), 0),
