-- Atomic, tenant-scoped import commits. Financial rows remain RPC-only.

create table if not exists public.import_batches (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.organizations(id) on delete cascade,
  import_key text not null,
  import_kind text not null check (import_kind in ('counterparties', 'opening_balances', 'debts')),
...
create policy import_batches_member_read on public.import_batches for select using (public.is_org_member(organization_id));
...
declare
  ...
  import_key text := nullif(trim(command->>'import_key'), '');
...
begin
  ...
  if import_key is null or length(import_key) > 128 then raise exception 'import_key is required'; end if;
  ...
  perform pg_advisory_xact_lock(hashtextextended(org_id::text || ':import:' || import_key, 0));
  select * into existing from public.import_batches where organization_id = org_id and import_key = commit_import.import_key;
  ...
  insert into public.import_batches (organization_id, import_key, import_kind, row_count, committed_by) values (org_id, import_key, kind, jsonb_array_length(rows), actor_id) returning id into batch_id;
  for row_item in select value from jsonb_array_elements(rows) loop
    ...
    if kind = 'counterparties' then
      ...
      result_entry := public.record_opening_balance(jsonb_build_object('organization_id', org_id, 'branch_id', row_item->>'branch_id', 'cashbox_id', row_item->>'cashbox_id', 'currency', row_item->>'currency', 'amount', row_item->>'amount', 'base_value', row_item->>'base_value', 'memo', 'Import ' || import_key || ' row ' || row_number, 'client_command_id', import_key || ':opening:' || row_number));
    else
      ...
      result_entry := public.record_debt(jsonb_build_object('organization_id', org_id, 'branch_id', row_item->>'branch_id', 'counterparty_id', counterparty_id, 'direction', row_item->>'direction', 'currency', row_item->>'currency', 'amount', row_item->>'amount', 'location', row_item->>'location', 'memo', 'Import ' || import_key || ' row ' || row_number, 'client_command_id', import_key || ':debt:' || row_number));
    end if;
