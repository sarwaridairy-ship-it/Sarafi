-- Harden non-FX operation posting and preserve accounting semantics.
...
begin
  ...
  if kind in ('RECORD_EXPENSE', 'PAY_MONEY', 'OWNER_WITHDRAWAL', 'BANK_DEPOSIT') then
    cash_debit := false;
  else
    cash_debit := true;
  end if;
  ...
  insert into public.ledger_accounts (organization_id, code, name, category, currency_code)
    ...
    on conflict (organization_id, code) do update set active = true returning id into offset_account;
