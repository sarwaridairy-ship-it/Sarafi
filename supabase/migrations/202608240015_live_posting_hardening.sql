-- Harden live posting authorization and keep the dashboard cost state rebuildable.
...
begin
  ...
  select je.* into existing_entry
    ...
    where fe.organization_id = org_id and fe.client_command_id = client_id
    limit 1;
  ...
  select id, role_code into membership_id_value, role_value
    from public.organization_memberships
    where organization_id = org_id and user_id = actor_id and active = true;
  ...
  insert into public.ledger_accounts (organization_id, code, name, category, currency_code, cashbox_id)
    values (org_id, 'inventory:' || sold_currency_value, 'FX inventory ' || sold_currency_value, 'asset', sold_currency_value, null)
    on conflict (organization_id, code) do update set active = true returning id into sold_account;
  insert into public.ledger_accounts (organization_id, code, name, category, currency_code, cashbox_id)
    values (org_id, 'inventory:' || bought_currency_value, 'FX inventory ' || bought_currency_value, 'asset', bought_currency_value, null)
    on conflict (organization_id, code) do update set active = true returning id into bought_account;
  insert into public.ledger_accounts (organization_id, code, name, category, currency_code, cashbox_id)
    values (org_id, 'cashbox:' || cashbox_id_value || ':' || bought_currency_value, 'Cashbox ' || bought_currency_value, 'asset', bought_currency_value, cashbox_id_value)
    on conflict (organization_id, code) do update set active = true returning id into cashbox_account;
  if side_value = 'buy_fx' then
    insert into public.ledger_accounts (organization_id, code, name, category, currency_code, cashbox_id)
      values (org_id, 'cashbox:' || cashbox_id_value || ':' || sold_currency_value, 'Cashbox ' || sold_currency_value, 'asset', sold_currency_value, cashbox_id_value)
      on conflict (organization_id, code) do update set active = true returning id into cashbox_account;
  end if;

  if side_value in ('sell_fx', 'exchange_fx') then
    select coalesce(sum(jl.native_debit - jl.native_credit), 0), coalesce(sum(jl.base_debit - jl.base_credit), 0)
      into available_quantity, carrying_cost
      from public.journal_lines jl
      where jl.organization_id = org_id and jl.account_id = sold_account and jl.currency_code = sold_currency_value;
  ...
  if side_value = 'buy_fx' then
    ...
    update public.fx_inventory_cost_state
      set quantity = quantity - sold_amount_value, carrying_base_value = carrying_base_value - carrying_cost, updated_at = now()
