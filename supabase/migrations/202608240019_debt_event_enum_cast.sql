-- Fix enum casts in debt event creation discovered by remote schema lint.
...
begin
  ...
  insert into public.financial_events (organization_id, branch_id, event_type, immutable_reference, occurred_at, created_by, client_command_id, metadata)
    values (org_id, branch_id_value, (case when direction_value = 'receivable' then 'receive_money' else 'pay_money' end)::public.financial_event_type, 'debt-' || client_id, coalesce((command->>'occurred_at')::timestamptz, now()), actor_id, client_id, command) returning id into event_id;
  insert into public.journal_entries (organization_id, branch_id, financial_event_id, status, occurred_at, posted_at, created_by, posted_by, memo)
    values (org_id, branch_id_value, event_id, 'posted', coalesce((command->>'occurred_at')::timestamptz, now()), now(), actor_id, actor_id, command->>'memo') returning id into entry_id;
  ...
  insert into public.ledger_accounts (organization_id, code, name, category, currency_code) values (org_id, direction_value || ':' || counterparty_id_value || ':' || currency_value, initcap(direction_value) || ' ' || currency_value, case when direction_value = 'receivable' then 'asset' else 'liability' end, currency_value) on conflict (organization_id, code) do update set active = true returning id into debt_account;
  ...
  insert into public.debts (organization_id, branch_id, counterparty_id, direction, currency_code, original_amount, outstanding_amount, originating_entry_id, due_at, notes) values (org_id, branch_id_value, counterparty_id_value, direction_value, currency_value, amount_value, amount_value, entry_id, (command->>'due_at')::timestamptz, command->>'memo');
...
begin
  ...
  insert into public.financial_events (organization_id, branch_id, event_type, immutable_reference, occurred_at, created_by, client_command_id, metadata)
    values (debt.organization_id, debt.branch_id, (case when debt.direction = 'receivable' then 'receive_money' else 'pay_money' end)::public.financial_event_type, 'settlement-' || client_id, now(), actor_id, client_id, command) returning id into event_id;
  insert into public.journal_entries (organization_id, branch_id, financial_event_id, status, occurred_at, posted_at, created_by, posted_by, memo) values (debt.organization_id, debt.branch_id, event_id, 'posted', now(), now(), actor_id, actor_id, command->>'memo') returning id into entry_id;
  insert into public.ledger_accounts (organization_id, code, name, category, currency_code) values (debt.organization_id, 'location:' || location_value || ':' || debt.currency_code, 'Location ' || debt.currency_code, 'asset', debt.currency_code) on conflict (organization_id, code) do update set active = true returning id into cash_account;
  insert into public.ledger_accounts (organization_id, code, name, category, currency_code) values (debt.organization_id, debt.direction || ':' || debt.counterparty_id || ':' || debt.currency_code, initcap(debt.direction) || ' ' || debt.currency_code, case when debt.direction = 'receivable' then 'asset' else 'liability' end, debt.currency_code) on conflict (organization_id, code) do update set active = true returning id into debt_account;
