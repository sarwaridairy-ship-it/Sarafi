...
import { downloadPdf, printReport, printThermalReceipt, shareReportViaWhatsApp } from './lib/exports'
...
import { OfflineOutbox } from './lib/offline'
...
function App() {
  ...
  const [rate, setRate] = useState(inspectionMode ? '70.25' : '')
  ...
  useEffect(() => {
    ...
    void getCurrentRates(organizationId, branchId ?? undefined).then((result) => {
      ...
      if (current) { setRate(current.buy_rate); setSellRate(current.sell_rate) }
    })
  ...
  return (
    <div className={`app-shell ${isRtl(language) ? 'rtl' : ''}`}>
      <aside className="sidebar">
        ...
        <nav>{(['Dashboard', 'Trade', 'Transactions', 'Cash & Accounts', 'People', 'Debts', 'Rates', 'Reports', 'Reconciliation', 'Offline'] as const).filter((item) => item === 'Offline' || ownerNavigation || ['Dashboard', 'Trade', 'Transactions', 'People'].includes(item)).map((item) => { const labels = { Dashboard: t('dashboard'), Trade: t('trade'), Transactions: t('transactions'), 'Cash & Accounts': t('cashAccounts'), People: t('people'), Debts: t('debts'), Rates: t('rates'), Reports: t('reports'), Reconciliation: t('reconciliation'), Offline: 'Offline' }; return <button className={activeNav === item ? 'nav-item active' : 'nav-item'} key={item} onClick={() => { openSection(item); if (item === 'Trade') setShowTrade(true) }}><span className="nav-icon">{['◫', '+', '≡', '▣', '♙', '↔', '↗', '▤', '✓', '⇄'][['Dashboard', 'Trade', 'Transactions', 'Cash & Accounts', 'People', 'Debts', 'Rates', 'Reports', 'Reconciliation', 'Offline'].indexOf(item)]}</span>{labels[item]}{item === 'Transactions' && <em>{dashboard?.transaction_count ?? '—'}</em>}</button> })}{(workspaceRole === 'owner' || workspaceRole === 'compliance_officer') && <button className={activeNav === 'Compliance' ? 'nav-item active' : 'nav-item'} onClick={() => openSection('Compliance')}><span className="nav-icon">!</span>Compliance</button>}</nav>
      ...
      <main className="main-content">
        ...
        <div className="content-wrap">
          ...
          <section className="rate-strip"><div className="rate-title"><span className="rate-live" /> <div><b>{t('rates')}</b><small>{t('retailRateContext')}</small></div></div><label>{t('buy')}<input value={rate} onChange={(event) => setRate(event.target.value)} placeholder="Live rate" /></label><label>{t('sell')}<input value={sellRate} readOnly placeholder="Live rate" /></label><div className="calculator"><input value={calculatorAmount} onChange={(event) => setCalculatorAmount(event.target.value)} /><span>USD</span><b>=</b><strong>{rate ? calculateCounterAmount(calculatorAmount || '0', rate, 'AFN_PER_UNIT', 2) : '—'}</strong><span>AFN</span></div><label>Business date<input type="date" value={dashboardDate} onChange={(event) => setDashboardDate(event.target.value)} /></label><button className="text-button" onClick={() => openSection('Rates')}>{t('history')} →</button></section>
          <section className="metric-grid"><article className="metric-card hero-metric"><div className="card-head"><span>Net position AFN</span><button aria-label={privacy ? 'Show amounts' : 'Hide amounts'} onClick={() => setPrivacy(!privacy)}>◌</button></div><strong>{hidden || dashboard?.net_position_base || '—'}</strong><div className="metric-foot"><span>Ledger-derived assets less liabilities</span></div><div className="sparkline">{Array.from({ length: 12 }, (_, index) => <i key={index} />)}</div></article><article className="metric-card"><div className="card-head"><span>{t('todayVolume')}</span><span className="card-symbol">↗</span></div><strong>{hidden || dashboard?.volume_base || '—'}</strong><div className="metric-foot"><span>{dashboard ? `${dashboard.transaction_count} posted entries` : 'Awaiting live ledger data'}</span></div></article><article className="metric-card"><div className="card-head"><span>{t('realizedProfit')}</span><span className="card-symbol profit">✦</span></div><strong className="profit-text">{hidden || dashboard?.realized_profit || '—'}</strong><div className="metric-foot"><span>Ledger-derived AFN equivalent</span></div></article><article className="metric-card"><div className="card-head"><span>Commission income</span><span className="card-symbol profit">✦</span></div><strong className="profit-text">{hidden || dashboard?.commission_income || '—'}</strong><div className="metric-foot"><span>Ledger-derived AFN equivalent</span></div></article><article className="metric-card"><div className="card-head"><span>Operating expenses</span><span className="card-symbol">↘</span></div><strong>{hidden || dashboard?.expenses || '—'}</strong><div className="metric-foot"><span>Ledger-derived AFN equivalent</span></div></article></section>
          <section className="live-business"><div><span className="live-pulse" /> <b>{t('liveBusiness')}</b><small>{dashboard ? `Fresh ${new Date(dashboard.fresh_at).toLocaleTimeString()}` : t('awaitingLiveLedger')}</small></div><div><strong>{dashboard?.pending_approvals ?? '—'}</strong><small>{t('pending')}</small></div><div><strong>{dashboard?.reconciliation_differences ?? '—'}</strong><small>Reconciliation difference</small></div><div><strong>{dashboard?.net_result ?? '—'}</strong><small>Net result AFN</small></div><button onClick={() => setDashboardRefresh((value) => value + 1)}>{t('refreshLiveView')} →</button></section>
...
function OfflineView({ organizationId, userId, deviceId, cashboxId, onDashboard }: { organizationId: string | null; userId: string; deviceId: string; cashboxId: string; onDashboard: () => void }) {
  const [commands, setCommands] = useState<ReturnType<OfflineOutbox['all']>>([])
  const [amount, setAmount] = useState('')
  const [currency, setCurrency] = useState('USD')
  const [kind, setKind] = useState<'BUY_FX' | 'SELL_FX'>('BUY_FX')
  const [message, setMessage] = useState('')
  const [outbox] = useState(() => new OfflineOutbox({ tenantId: organizationId ?? 'unknown', userId, deviceId, cashboxId, maxAmountBase: '100000', allowKinds: ['BUY_FX', 'SELL_FX'] }, indexedDbOfflineStore))
  useEffect(() => { void outbox.hydrate().then(() => setCommands(outbox.all())).catch((error) => setMessage(error instanceof Error ? `Queue unavailable: ${error.message}` : 'Queue unavailable')) }, [outbox])
  const enqueue = (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); try { const command = outbox.enqueue({ tenantId: organizationId ?? 'unknown', userId, deviceId, cashboxId, amount, currency, kind }); setCommands(outbox.all()); setAmount(''); setMessage(`Queued ${command.clientCommandId}; it is not posted.`) } catch (error) { setMessage(error instanceof Error ? error.message : 'Command rejected') } }
  return <section className="panel"><div className="panel-header"><div><p className="kicker">BAD-NETWORK CONTROL</p><h1>Offline queue</h1><p>Only policy-approved FX commands can wait for reconnect. Every item remains encrypted and identity-bound.</p></div><button className="text-button" onClick={onDashboard}>Back to dashboard →</button></div><div className="notice"><span className="sync-dot offline" /><span><b>Offline-safe mode</b> · queued commands are never labelled posted</span></div><form className="trade-modal" onSubmit={enqueue}><div className="form-grid"><label>Operation<select value={kind} onChange={(event) => setKind(event.target.value as typeof kind)}><option value="BUY_FX">Buy FX</option><option value="SELL_FX">Sell FX</option></select></label><label>Currency<select value={currency} onChange={(event) => setCurrency(event.target.value)}><option>USD</option><option>EUR</option><option>AFN</option></select></label></div><label>Amount<input required min="0.01" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0.00" /></label><button className="primary-action full" type="submit">Queue command <span>→</span></button></form>{message && <p role="status" className="empty-live">{message}</p>}<div className="balance-list">{commands.length ? commands.map((command) => <div className="balance-row" key={command.clientCommandId}><span className="currency-badge usd">{command.currency}</span><span className="balance-name"><b>{command.kind.replace('_FX', ' FX')} · {command.status}</b><small>{command.clientCommandId} · sequence {command.localSequence}</small></span><strong>{command.amount}</strong></div>) : <div className="empty-live">No offline commands are stored for this identity.</div>}</div><div className="empty-live">Reconnect processing re-validates authorization, device status, inventory, rates, limits, shift state, and idempotency on the server.</div></section>
}
...
function ReportsView({ trades, onDashboard, onToast }: { trades: Trade[]; onDashboard: () => void; onToast: (message: string) => void }) {
  ...
  const share = async () => { const allowed = await authorizeExport('print'); if (allowed) { shareReportViaWhatsApp({ reportName: 'Recent Activity', reference: filteredRows[0]?.entryId ?? 'snapshot', businessName: 'Kabul Central Exchange' }); onToast('WhatsApp share opened') } }
