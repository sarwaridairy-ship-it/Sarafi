  ...
  return (
    <div className={`app-shell ${isRtl(language) ? 'rtl' : ''}`}>
      <aside className="sidebar">
        ...
        <nav>{(['Dashboard', 'Trade', 'Transactions', 'Cash & Accounts', 'People', 'Debts', 'Rates', 'Reports', 'Reconciliation'] as const).filter((item) => ownerNavigation || ['Dashboard', 'Trade', 'Transactions', 'People'].includes(item)).map((item) => { const labels = { Dashboard: t('dashboard'), Trade: t('trade'), Transactions: t('transactions'), 'Cash & Accounts': t('cashAccounts'), People: t('people'), Debts: t('debts'), Rates: t('rates'), Reports: t('reports'), Reconciliation: t('reconciliation') }; return <button className={activeNav === item ? 'nav-item active' : 'nav-item'} key={item} onClick={() => { openSection(item); if (item === 'Trade') setShowTrade(true) }}><span className="nav-icon">{['◫', '+', '≡', '▣', '♙', '↔', '↗', '▤', '✓'][['Dashboard', 'Trade', 'Transactions', 'Cash & Accounts', 'People', 'Debts', 'Rates', 'Reports', 'Reconciliation'].indexOf(item)]}</span>{labels[item]}{item === 'Transactions' && <em>{dashboard?.transaction_count ?? '—'}</em>}</button> })}{(workspaceRole === 'owner' || workspaceRole === 'compliance_officer') && <button className={activeNav === 'Compliance' ? 'nav-item active' : 'nav-item'} onClick={() => openSection('Compliance')}><span className="nav-icon">!</span>Compliance</button>}</nav>
