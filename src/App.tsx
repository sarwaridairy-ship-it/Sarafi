...
function App() {
  ...
  const addTrade = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setTrades([{ id: Date.now(), customer: 'Authenticated customer', direction: 'USD -> AFN', amount: `$${amount}`, rate: '70.25', time: 'Now', status: 'Posted' }, ...trades])
    setAmount('')
    setShowTrade(false)
    ...
    <div className={`app-shell ${isRtl(language) ? 'rtl' : ''}`}>
      ...
      <main className="main-content">
        ...
        <div className="content-wrap">
          ...
          <section className="metric-grid"><article className="metric-card hero-metric"><div className="card-head"><span>Total position</span><button aria-label="Hide amount" onClick={() => setPrivacy(!privacy)}>◌</button></div><strong>{hidden || '—'}</strong><div className="metric-foot"><span>Awaiting live ledger data</span></div><div className="sparkline">{Array.from({ length: 12 }, (_, index) => <i key={index} />)}</div></article><article className="metric-card"><div className="card-head"><span>Today&apos;s volume</span><span className="card-symbol">↗</span></div><strong>{hidden || '—'}</strong><div className="metric-foot"><span>Awaiting live ledger data</span></div></article><article className="metric-card"><div className="card-head"><span>Realized profit</span><span className="card-symbol profit">✦</span></div><strong className="profit-text">{hidden || '—'}</strong><div className="metric-foot"><span>Awaiting live ledger data</span></div></article></section>
