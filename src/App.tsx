...
function App() {
  ...
  useEffect(() => {
    ...
    const plainTechnicalCopy: Record<string, string> = language === 'fa-AF'
      ? { 'LEDGER OPERATION': 'عملیات صرافی', 'Review posted ledger activity and transaction history.': 'فعالیت ثبت‌شده و تاریخچه معاملات را ببینید.', 'Ledger lines available': 'معاملات ثبت‌شده', 'Contributing ledger lines': 'منبع این مبلغ', 'Every amount traces back to a posted journal entry.': 'هر مبلغ به یک معامله ثبت‌شده مربوط است.', 'The server validates authorization, tenant scope, and ledger posting before accepting this operation.': 'سرافی پیش از ثبت، اجازه، تجارت و جزئیات پول را بررسی می‌کند.', 'Roles, MFA assurance, device status, and approval history from the authoritative control plane.': 'افراد مجاز، دستگاه‌ها و درخواست‌های نیازمند تأیید را ببینید.', 'Self-approval is rejected by the database constraint and RPC.': 'هر شخص نمی‌تواند درخواست خودش را تأیید کند.', 'Role checks, branch/cashbox scope, revoked-device status, and self-approval rules remain authoritative on Supabase; this screen never grants access client-side.': 'اجازه، شعبه، صندوق، دستگاه و تأیید هنگام ثبت هر عملیات به‌صورت خودکار بررسی می‌شود.' }
      : language === 'ps-AF'
        ? { 'LEDGER OPERATION': 'د صرافۍ کړنه', 'Review posted ledger activity and transaction history.': 'ثبت شوی فعالیت او د معاملو تاریخچه وګورئ.', 'Ledger lines available': 'ثبت شوې معاملې', 'Contributing ledger lines': 'د دې مبلغ سرچینه', 'Every amount traces back to a posted journal entry.': 'هر مبلغ له یوې ثبت شوې معاملې سره تړلی دی.', 'The server validates authorization, tenant scope, and ledger posting before accepting this operation.': 'صرافي له ثبتولو مخکې اجازه، کاروبار او د پیسو معلومات ګوري.', 'Roles, MFA assurance, device status, and approval history from the authoritative control plane.': 'د کاروونکو اجازه، وسایل او د تایید غوښتنې وګورئ.', 'Self-approval is rejected by the database constraint and RPC.': 'هر څوک خپله غوښتنه نه شي تاییدولی.', 'Role checks, branch/cashbox scope, revoked-device status, and self-approval rules remain authoritative on Supabase; this screen never grants access client-side.': 'د اجازې، څانګې، صندوق، وسیلې او تایید قواعد د هر ثبت پر مهال په خپله کتل کېږي.' }
        : { 'LEDGER OPERATION': 'SHOP ACTION', 'Review posted ledger activity and transaction history.': 'Review recorded activity and transaction history.', 'Ledger lines available': 'Posted records', 'Contributing ledger lines': 'Where this amount came from', 'Every amount traces back to a posted journal entry.': 'Each amount is linked to a recorded transaction.', 'The server validates authorization, tenant scope, and ledger posting before accepting this operation.': 'SARAFI checks your access, shop, and money details before saving this action.', 'Roles, MFA assurance, device status, and approval history from the authoritative control plane.': 'See who can use the shop, which devices are active, and what needs approval.', 'Self-approval is rejected by the database constraint and RPC.': 'One person cannot approve their own request.', 'Role checks, branch/cashbox scope, revoked-device status, and self-approval rules remain authoritative on Supabase; this screen never grants access client-side.': 'Access, branch, cashbox, device, and approval rules are checked automatically when an action is saved.' }
    Object.assign(replacements, plainTechnicalCopy)
...
function WorkspaceView({ section, trades, organizationId, userId, deviceId, branchId, cashboxId, onDashboard, onToast }: { section: string; trades: Trade[]; organizationId: string | null; userId: string; deviceId: string; branchId: string | null; cashboxId: string | null; onDashboard: () => void; onToast: (message: string) => void }) {
  ...
  const descriptions: Record<string, string> = {
    Transactions: 'Review recorded activity and transaction history.',
