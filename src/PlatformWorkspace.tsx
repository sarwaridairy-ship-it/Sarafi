...
import {
  ...
  getSubscriptionPaymentReceiptUrl,
...
const copy: Record<Language, Copy> = {
  en: {
    ...
    workspaceOverview: 'Platform overview', businessProfile: 'Business profile', businessReference: 'Business ID', currentSubscription: 'Current subscription', renewalDate: 'Renews or ends', teamMembers: 'Team members', noUsers: 'No users are connected to this business.', securityVerifiedLabel: 'Administrator verified',
    ...
    referenceHelp: 'Enter the receipt or transfer reference', uploadReceipt: 'Upload payment receipt', receiptHelp: 'PDF, JPG, or PNG up to 6 MB', viewReceipt: 'View receipt', optionalNote: 'Optional note', requestActivation: 'Send for activation', requestSent: 'Payment was sent for administrator review.',
    ...
    planCatalog: 'Plan catalog', published: 'Available', draft: 'Draft', retired: 'Retired', includedServices: 'Included controls', activePrice: 'Offer this duration', savePrice: 'Save price',
  ...
  'fa-AF': {
    ...
    workspaceOverview: 'نمای عمومی سیستم', businessProfile: 'مشخصات صرافی', businessReference: 'شناسه صرافی', currentSubscription: 'بسته فعلی', renewalDate: 'تمدید یا ختم', teamMembers: 'کارمندان صرافی', noUsers: 'هیچ کاربری به این صرافی وصل نیست.', securityVerifiedLabel: 'مدیر تأیید شده',
    ...
    referenceHelp: 'شماره رسید یا انتقال را بنویسید', uploadReceipt: 'آپلود عکس یا فایل رسید', receiptHelp: 'PDF، JPG یا PNG تا ۶ MB', viewReceipt: 'دیدن رسید', optionalNote: 'یادداشت اختیاری', requestActivation: 'فرستادن برای فعال‌سازی', requestSent: 'پرداخت برای بررسی مدیر فرستاده شد.',
    ...
    planCatalog: 'فهرست بسته‌ها', published: 'قابل استفاده', draft: 'پیش‌نویس', retired: 'بازنشسته', includedServices: 'خدمات شامل', activePrice: 'این مدت قابل انتخاب باشد', savePrice: 'ذخیره قیمت',
  ...
  'ps-AF': {
    ...
    workspaceOverview: 'د سیسټم لنډیز', businessProfile: 'د صرافۍ پېژندنه', businessReference: 'د صرافۍ شمېره', currentSubscription: 'اوسنۍ بسته', renewalDate: 'نوي کېدل یا پای', teamMembers: 'د صرافۍ کارکوونکي', noUsers: 'له دې صرافۍ سره کاروونکي نه دي نښلول شوي.', securityVerifiedLabel: 'مدیر تایید شوی',
    ...
    referenceHelp: 'د رسید یا لېږد شمېره ولیکئ', uploadReceipt: 'د تادیې رسید پورته کړئ', receiptHelp: 'PDF، JPG یا PNG تر ۶ MB', viewReceipt: 'رسید کتل', optionalNote: 'اختیاري یادښت', requestActivation: 'د فعالولو لپاره لېږل', requestSent: 'تادیه د مدیر کتنې ته ولېږل شوه.',
    ...
    planCatalog: 'د بستو لېست', published: 'د کار وړ', draft: 'مسوده', retired: 'تړل شوې', includedServices: 'شامل خدمتونه', activePrice: 'دا موده د ټاکلو وړ وي', savePrice: 'بیه ساتل',
...
const subscriptionTerms = [1, 3, 6, 12] as const
type SubscriptionTerm = (typeof subscriptionTerms)[number]
type PlanPriceDraft = { price: string; active: boolean }
const planPriceKey = (planId: string, term: SubscriptionTerm) => `${planId}:${term}`
const createPlanPriceDrafts = (plans: SubscriptionPlan[]) => Object.fromEntries(plans.flatMap((plan) => subscriptionTerms.map((term) => {
  const current = plan.term_prices.find((price) => price.term_months === term)
  return [planPriceKey(plan.id, term), { price: current?.price_afn ?? '', active: current?.active ?? Boolean(current) } satisfies PlanPriceDraft]
}))) as Record<string, PlanPriceDraft>
...
export function BillingView({
  ...
  const load = useCallback(async () => {
    if (inspection) {
      setPortal({
        ...
        plans: [
          { id: 'starter', code: 'starter', name_en: 'Small shop', name_dari: 'صرافی کوچک', name_pashto: 'کوچنۍ صرافي', description_en: 'One branch with the essential daily tools.', description_dari: 'یک شعبه با ابزارهای ضروری روزانه.', description_pashto: 'یوه څانګه او د ورځني کار اړین وسایل.', price_afn: '1000', term_prices: [{ term_months: 1, price_afn: '1000' }, { term_months: 3, price_afn: '2850' }, { term_months: 6, price_afn: '5400' }, { term_months: 12, price_afn: '10000' }], billing_interval: 'monthly', employee_limit: 5, branch_limit: 1, features: {}, status: 'published', sort_order: 10 },
          { id: 'business', code: 'business', name_en: 'Growing business', name_dari: 'صرافی در حال رشد', name_pashto: 'پراخېدونکې صرافي', description_en: 'More employees, branches, reports, and compliance controls.', description_dari: 'کارمندان، شعبه‌ها، گزارش‌ها و کنترول بیشتر.', description_pashto: 'ډېر کارکوونکي، څانګې، راپورونه او کنترول.', price_afn: '2500', term_prices: [{ term_months: 1, price_afn: '2500' }, { term_months: 3, price_afn: '7125' }, { term_months: 6, price_afn: '13500' }, { term_months: 12, price_afn: '25000' }], billing_interval: 'monthly', employee_limit: 25, branch_limit: 5, features: {}, status: 'published', sort_order: 20 },
        ],
        ...
        requests: [{ id: 'preview-payment', plan_id: 'starter', provider_code: 'manual_review', amount_afn: '2850', term_months: 3, payer_reference: 'PAY-1042', status: 'pending', requested_at: new Date().toISOString(), receipt_storage_path: 'inspection/receipt.jpg', receipt_file_name: 'payment-receipt.jpg', receipt_mime_type: 'image/jpeg' }],
      })
      setPlanId('starter')
      return
    }
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!receipt) { onToast(c.failed); return }
    if (inspection) { onToast(c.requestSent); setReference(''); setNote(''); setReceipt(null); return }
    setBusy(true)
    const result = await createSubscriptionPaymentRequest({ organizationId, planId, providerCode, termMonths, reference, note, receipt })
    setBusy(false)
  ...
  const viewReceipt = async (receiptPath: string) => {
    if (inspection) { onToast(c.viewReceipt); return }
    const result = await getSubscriptionPaymentReceiptUrl(receiptPath)
    if (!result.data) { onToast(c.failed); return }
  ...
  const selectedPlan = portal.plans.find((plan) => plan.id === planId)
  const selectedTermPrice = selectedPlan?.term_prices.find((price) => price.term_months === termMonths)
  return (
    <section className="panel billing-workspace">
      ...
      <form onSubmit={submit} className="billing-form">
        ...
        <fieldset><legend>{c.chooseTerm}</legend><div className="subscription-term-grid">
          {([1, 3, 6, 12] as const).map((term) => {
            const price = selectedPlan?.term_prices.find((item) => item.term_months === term)
            return <label className={termMonths === term ? 'selected' : ''} key={term}>
              <input type="radio" name="term" value={term} checked={termMonths === term} onChange={() => setTermMonths(term)} />
              <b>{term} {c.months}</b><span dir="ltr">{price?.price_afn ?? '—'} AFN</span>
            </label>
          })}
        </div><p className="payment-instructions">{c.totalPrice}: <b dir="ltr">{selectedTermPrice?.price_afn ?? '—'} AFN</b></p></fieldset>
        ...
        <label className="payment-receipt-field">{c.uploadReceipt}<input required type="file" accept="application/pdf,image/jpeg,image/png" onChange={(event) => setReceipt(event.target.files?.[0] ?? null)} /><small>{c.receiptHelp}</small></label>
        <label>{c.optionalNote}<textarea value={note} onChange={(event) => setNote(event.target.value)} /></label>
        <button className="primary-action" disabled={busy || !planId || !providerCode || !selectedTermPrice || !receipt}>{c.requestActivation}</button>
      </form>
      <section className="payment-history"><h2>{c.paymentHistory}</h2>{portal.requests.length ? portal.requests.map((request) => <div className="balance-row" key={request.id}><span className="currency-badge usd">؋</span><span className="balance-name"><b>{request.amount_afn} AFN · {request.term_months} {c.months}</b><small>{request.payer_reference} · {new Date(request.requested_at).toLocaleDateString(language)}</small>{request.receipt_file_name ? <small>{request.receipt_file_name}</small> : null}</span><span className="payment-history-actions"><strong>{statusText(language, request.status)}</strong>{request.receipt_storage_path ? <button className="text-button" type="button" onClick={() => void viewReceipt(request.receipt_storage_path!)}>{c.viewReceipt}</button> : null}</span></div>) : <div className="empty-live">{c.noRequests}</div>}</section>
    </section>
  )
}
...
export function PlatformAdminConsole({
  ...
  const [planPriceDrafts, setPlanPriceDrafts] = useState<Record<string, PlanPriceDraft>>({})
  ...
  const load = useCallback(async () => {
    ...
    if (inspection) {
      const previewPlans: SubscriptionPlan[] = [
        { id: 'starter', code: 'starter', name_en: 'Small shop', name_dari: 'صرافی کوچک', name_pashto: 'کوچنۍ صرافي', description_en: 'One branch with essential daily tools.', description_dari: 'یک شعبه با ابزارهای ضروری روزانه.', description_pashto: 'یوه څانګه او د ورځني کار اړین وسایل.', price_afn: '1000', term_prices: [{ term_months: 1, price_afn: '1000' }, { term_months: 3, price_afn: '2850' }, { term_months: 6, price_afn: '5400' }, { term_months: 12, price_afn: '10000' }], billing_interval: 'monthly', employee_limit: 5, branch_limit: 1, features: { reports: true, team: true }, status: 'published', sort_order: 10 },
        { id: 'business', code: 'business', name_en: 'Growing business', name_dari: 'صرافی در حال رشد', name_pashto: 'پراخېدونکې صرافي', description_en: 'More employees, branches, reports, and compliance controls.', description_dari: 'کارمندان، شعبه‌ها، گزارش‌ها و کنترول بیشتر.', description_pashto: 'ډېر کارکوونکي، څانګې، راپورونه او کنترول.', price_afn: '2500', term_prices: [{ term_months: 1, price_afn: '2500' }, { term_months: 3, price_afn: '7125' }, { term_months: 6, price_afn: '13500' }, { term_months: 12, price_afn: '25000' }], billing_interval: 'monthly', employee_limit: 25, branch_limit: 5, features: { reports: true, team: true, compliance: true }, status: 'published', sort_order: 20 },
      ]
      setData({ organizations: [{ id: 'preview-org', business_number: 1042, business_reference: 'SARAFI-001042', display_name: 'Kabul Central Exchange', created_at: new Date().toISOString(), member_count: 6, plan_code: 'business', subscription_status: 'active', period_end: new Date(Date.now() + 86400000 * 30).toISOString() }], plans: previewPlans, providers: [{ code: 'manual_review', name_en: 'Bank or office payment', name_dari: 'پرداخت بانکی یا در دفتر', name_pashto: 'بانکي یا د دفتر تادیه', instructions_en: 'Review the receipt reference before activation.', instructions_dari: 'پیش از فعال‌سازی شماره رسید را بررسی کنید.', instructions_pashto: 'له فعالولو مخکې د رسید شمېره وګورئ.', provider_mode: 'manual_review', state: 'live', public_checkout_url: null }], payment_requests: [{ id: 'preview-payment', organization_id: 'preview-org', organization_name: 'Kabul Central Exchange', plan_id: 'business', plan_name: 'Growing business', provider_code: 'manual_review', amount_afn: '7125', term_months: 3, payer_reference: 'PAY-1042', status: 'pending', requested_at: new Date().toISOString(), receipt_storage_path: 'inspection/receipt.jpg', receipt_file_name: 'payment-receipt.jpg', receipt_mime_type: 'image/jpeg' }], audit_events: [{ id: 'preview-audit', event_type: 'subscription_payment_approved', target_organization_id: 'preview-org', organization_name: 'Kabul Central Exchange', target_user_id: null, created_at: new Date().toISOString() }], counts: { organizations: 1, active_subscriptions: 1, pending_payments: 1, suspended_users: 0 } })
      setSelectedOrganization('preview-org')
      setPlanPriceDrafts(createPlanPriceDrafts(previewPlans))
    ...
    setData(result.data)
    if (result.data) {
      setPlanPriceDrafts(createPlanPriceDrafts(result.data.plans))
      setSelectedOrganization((current) => current && result.data?.organizations.some((item) => item.id === current) ? current : result.data?.organizations[0]?.id ?? null)
    }
  ...
  useEffect(() => {
    ...
    if (inspection) {
      setUsers([
        { user_id: 'preview-owner', user_number: 1001, user_reference: 'USR-001001', membership_id: 'preview-membership-owner', display_name: 'Ahmad Safi', email: 'owner@example.com', role_code: 'owner', membership_active: true, platform_status: 'active', platform_reason: null, joined_at: new Date().toISOString() },
        { user_id: 'preview-cashier', user_number: 1002, user_reference: 'USR-001002', membership_id: 'preview-membership-cashier', display_name: 'Mariam Rahimi', email: 'cashier@example.com', role_code: 'cashier', membership_active: true, platform_status: 'active', platform_reason: null, joined_at: new Date().toISOString() },
      ])
      return
    }
  ...
  }, [c.failed, inspection, selectedOrganization])
  ...
  const activeOrganization = data?.organizations.find((item) => item.id === selectedOrganization) ?? null
  const activePlan = activeOrganization ? data?.plans.find((plan) => plan.code === activeOrganization.plan_code) ?? null : null
  ...
  const viewPaymentReceipt = async (receiptPath: string) => {
    if (inspection) { setToast(c.viewReceipt); return }
    const result = await getSubscriptionPaymentReceiptUrl(receiptPath)
    if (!result.data) { setToast(c.failed); return }
    window.open(result.data, '_blank', 'noopener,noreferrer')
  }
  const savePlanPrice = async (planId: string, term: SubscriptionTerm) => {
    const draft = planPriceDrafts[planPriceKey(planId, term)]
    if (!draft || !draft.price || Number(draft.price) <= 0) { setToast(c.failed); return }
    if (inspection) {
      setData((current) => current ? {
        ...current,
        plans: current.plans.map((plan) => plan.id === planId ? {
          ...plan,
          price_afn: term === 1 ? draft.price : plan.price_afn,
          term_prices: subscriptionTerms.map((candidate) => {
            const candidateDraft = planPriceDrafts[planPriceKey(plan.id, candidate)]
            return { term_months: candidate, price_afn: candidateDraft?.price || '0', active: candidateDraft?.active ?? false }
          }),
        } : plan),
      } : current)
      setToast(c.saved)
    ...
    await act(setSubscriptionPlanPrice({ planId, termMonths: term, priceAfn: draft.price, active: draft.active }))
  }
  ...
  return <main className="platform-shell">
    <header className="platform-header"><div><span className="brand-mark">S</span><span><b>{c.admin}</b><small>{c.adminIntro}</small></span></div><div><span className={`platform-verified-badge ${mfa.verified ? '' : 'pending'}`}>{mfa.verified ? `✓ ${c.securityVerifiedLabel}` : c.securityTitle}</span><select value={language} aria-label={c.language} onChange={(event) => onLanguageChange(event.target.value as Language)}><option value="fa-AF">دری</option><option value="ps-AF">پښتو</option><option value="en">English</option></select><button className="text-button" onClick={onSignOut}>{c.signOut}</button></div></header>
    <div className="platform-section-heading"><p className="kicker">{c.workspaceOverview}</p></div>
    <section className="platform-metrics"><article><span>{c.organizations}</span><b>{data.counts.organizations}</b></article><article><span>{c.activePlans}</span><b>{data.counts.active_subscriptions}</b></article><article className={data.counts.pending_payments ? 'attention' : ''}><span>{c.pendingPayments}</span><b>{data.counts.pending_payments}</b></article><article className={data.counts.suspended_users ? 'attention' : ''}><span>{c.suspendedUsers}</span><b>{data.counts.suspended_users}</b></article></section>
    <nav className="platform-tabs" aria-label={c.admin}><button type="button" aria-current={tab === 'businesses' ? 'page' : undefined} className={tab === 'businesses' ? 'active' : ''} onClick={() => setTab('businesses')}>{c.businesses}</button><button type="button" aria-current={tab === 'payments' ? 'page' : undefined} className={tab === 'payments' ? 'active' : ''} onClick={() => setTab('payments')}>{c.payments}</button><button type="button" aria-current={tab === 'providers' ? 'page' : undefined} className={tab === 'providers' ? 'active' : ''} onClick={() => setTab('providers')}>{c.providers}</button><button type="button" aria-current={tab === 'plans' ? 'page' : undefined} className={tab === 'plans' ? 'active' : ''} onClick={() => setTab('plans')}>{c.plans}</button><button type="button" aria-current={tab === 'operations' ? 'page' : undefined} className={tab === 'operations' ? 'active' : ''} onClick={() => setTab('operations')}>{c.operations}</button><button type="button" aria-current={tab === 'support' ? 'page' : undefined} className={tab === 'support' ? 'active' : ''} onClick={() => setTab('support')}>{c.support}</button><button type="button" aria-current={tab === 'announcements' ? 'page' : undefined} className={tab === 'announcements' ? 'active' : ''} onClick={() => setTab('announcements')}>{c.announcements}</button><button type="button" aria-current={tab === 'audit' ? 'page' : undefined} className={tab === 'audit' ? 'active' : ''} onClick={() => setTab('audit')}>{c.audit}</button></nav>
    ...
    {tab === 'businesses' && <section className="platform-grid">
      <aside className="platform-list">
        <div className="platform-list-heading"><span><b>{c.businesses}</b><small>{organizations.length} / {data.counts.organizations}</small></span></div>
        <label>{c.findBusiness}<input type="search" value={query} onChange={(event) => setQuery(event.target.value)} /></label>
        <div className="platform-business-list">{organizations.map((organization) => {
          const plan = data.plans.find((item) => item.code === organization.plan_code)
          return <button type="button" className={selectedOrganization === organization.id ? 'platform-business active' : 'platform-business'} key={organization.id} onClick={() => setSelectedOrganization(organization.id)}><span><b>{organization.display_name}</b><small>{organization.business_reference ? organization.business_reference + ' · ' : ''}{organization.member_count} {c.members} · {plan ? localized(language, plan, 'name') : '—'}</small></span><strong>{statusText(language, organization.subscription_status ?? 'trial')}</strong></button>
        })}{!organizations.length ? <div className="empty-live">{c.noUsers}</div> : null}</div>
      </aside>
      <div className="platform-detail">{activeOrganization ? <>
        <header className="platform-business-hero"><span><p className="kicker">{c.businessProfile}</p><h1>{activeOrganization.display_name}</h1><small>{activeOrganization.business_reference ?? (activeOrganization.business_number ? `SARAFI-${String(activeOrganization.business_number).padStart(6, '0')}` : activeOrganization.id)}</small></span><strong className={`subscription-state ${activeOrganization.subscription_status ?? 'trial'}`}>{statusText(language, activeOrganization.subscription_status ?? 'trial')}</strong></header>
        <div className="platform-business-summary">
          <article><span>{c.businessReference}</span><b dir="ltr">{activeOrganization.business_reference ?? activeOrganization.id}</b></article>
          <article><span>{c.currentSubscription}</span><b>{activePlan ? localized(language, activePlan, 'name') : '—'}</b></article>
          <article><span>{c.teamMembers}</span><b>{activeOrganization.member_count}</b></article>
          <article><span>{c.renewalDate}</span><b>{activeOrganization.period_end ? new Date(activeOrganization.period_end).toLocaleDateString(language) : '—'}</b></article>
        </div>
        <section className="platform-detail-section"><h2>{c.subscriptionControl}</h2><label>{c.reason}<input value={reason} onChange={(event) => setReason(event.target.value)} placeholder={c.reasonPlaceholder} /></label><div className="inline-actions"><button type="button" disabled={!mfa.verified} onClick={() => void act(setSubscriptionStatus({ organizationId: activeOrganization.id, status: 'active', reason: reason || c.restore }))}>{c.restore}</button><button type="button" disabled={!mfa.verified || !reason.trim()} className="danger" onClick={() => void act(setSubscriptionStatus({ organizationId: activeOrganization.id, status: 'suspended', reason }))}>{c.suspend}</button></div></section>
        <section className="platform-detail-section"><div className="platform-subsection-heading"><h2>{c.teamMembers}</h2><span>{users.length}</span></div>{users.length ? users.map((user) => <div className="platform-user" key={user.user_id}><span><b>{user.display_name}</b><small>{user.user_reference ? user.user_reference + ' · ' : ''}{user.email} · {roleText(language, user.role_code)}</small></span><strong>{statusText(language, user.platform_status)}</strong><button type="button" disabled={!mfa.verified} onClick={() => void act(setPlatformUserAccess({ userId: user.user_id, status: user.platform_status === 'active' ? 'suspended' : 'active', reason }))}>{user.platform_status === 'active' ? c.suspend : c.restore}</button></div>) : <div className="empty-live">{c.noUsers}</div>}</section>
      </> : <div className="empty-live">{c.open}</div>}</div>
    </section>}
    {tab === 'payments' && <section className="platform-panel"><h1>{c.paymentQueue}</h1><label>{c.adminNote}<input value={adminNote} onChange={(event) => setAdminNote(event.target.value)} placeholder={c.reasonPlaceholder} /></label>{data.payment_requests.length ? data.payment_requests.map((request) => <article className="payment-review-card" key={request.id}><div><h2>{request.organization_name}</h2><p>{request.plan_name} · {request.term_months} {c.months} · {request.amount_afn} AFN</p><small>{c.receipt}: {request.payer_reference || '—'} · {new Date(request.requested_at).toLocaleString(language)}</small>{request.receipt_file_name ? <small>{request.receipt_file_name}</small> : null}</div><div>{request.receipt_storage_path ? <button className="text-button" type="button" onClick={() => void viewPaymentReceipt(request.receipt_storage_path!)}>{c.viewReceipt}</button> : null}<button className="primary-action" onClick={() => void act(decideSubscriptionPayment({ requestId: request.id, decision: 'approved', note: adminNote }))}>{c.approve}</button><button className="danger" onClick={() => void act(decideSubscriptionPayment({ requestId: request.id, decision: 'rejected', note: adminNote }))}>{c.reject}</button></div></article>) : <div className="empty-live">{c.noPayments}</div>}</section>}
    ...
    {tab === 'plans' && <section className="platform-panel"><h1>{c.planCatalog}</h1><div className="platform-plan-grid">{data.plans.map((plan) => <article className="plan-choice selected platform-plan-editor" key={plan.id}><span className={`subscription-state ${plan.status}`}>{statusText(language, plan.status)}</span><h2>{localized(language, plan, 'name')}</h2><p>{localized(language, plan, 'description')}</p><div className="plan-term-editor">{subscriptionTerms.map((term) => { const key = planPriceKey(plan.id, term); const draft = planPriceDrafts[key] ?? { price: '', active: false }; return <form key={term} onSubmit={(event) => { event.preventDefault(); void savePlanPrice(plan.id, term) }}><b>{term} {c.months}</b><label>{c.amount}<input required min="1" step="1" inputMode="decimal" value={draft.price} onChange={(event) => setPlanPriceDrafts((current) => ({ ...current, [key]: { ...draft, price: event.target.value } }))} /><span>AFN</span></label><label className="switch-row"><input type="checkbox" checked={draft.active} onChange={(event) => setPlanPriceDrafts((current) => ({ ...current, [key]: { ...draft, active: event.target.checked } }))} /><span>{c.activePrice}</span></label><button className="text-button" disabled={!mfa.verified}>{c.savePrice}</button></form> })}</div><small>{plan.employee_limit} {c.employees} · {plan.branch_limit} {c.branches}</small><em>{c.includedServices}: {Object.entries(plan.features).filter(([, enabled]) => enabled).map(([name]) => planFeatureText(language, name)).join(' · ') || '—'}</em></article>)}</div></section>}
