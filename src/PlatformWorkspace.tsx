...
export function BillingView({
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setBusy(true)
    const result = await createSubscriptionPaymentRequest({ organizationId, planId, providerCode, termMonths, reference, note })
    setBusy(false)
  ...
  return (
    <section className="panel billing-workspace">
      ...
      <form onSubmit={submit} className="billing-form">
        ...
        <button className="primary-action" disabled={busy || !planId || !providerCode || !selectedTermPrice}>{c.requestActivation}</button>
      </form>
...
export function PlatformAdminConsole({
  ...
  return <main className="platform-shell">
    ...
    {tab === 'plans' && <section className="platform-panel"><h1>{c.planCatalog}</h1><div className="platform-plan-grid">{data.plans.map((plan) => <article className="plan-choice selected" key={plan.id}><span className={`subscription-state ${plan.status}`}>{statusText(language, plan.status)}</span><h2>{localized(language, plan, 'name')}</h2><p>{localized(language, plan, 'description')}</p><div className="plan-term-list">{plan.term_prices.map((price) => <b dir="ltr" key={price.term_months}>{price.term_months} {c.months} · {price.price_afn} AFN</b>)}</div><small>{plan.employee_limit} {c.employees} · {plan.branch_limit} {c.branches}</small><em>{c.includedServices}: {Object.entries(plan.features).filter(([, enabled]) => enabled).map(([name]) => planFeatureText(language, name)).join(' · ') || '—'}</em></article>)}</div></section>}
